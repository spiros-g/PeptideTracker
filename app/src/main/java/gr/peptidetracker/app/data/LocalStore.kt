...
class LocalStore(context: Context) {
    ...
    fun add(peptide:String,amount:String,note:String){val now=System.currentTimeMillis();val n=prefs.getStringSet("entries",emptySet()).orEmpty().toMutableSet();n.add("$now|${safe(peptide)}|${safe(amount)}|${safe(note)}|$now");prefs.edit().putStringSet("entries",n).apply()}
    ...
    fun addProgress(weight:Double,waist:Double?,note:String){val now=System.currentTimeMillis();val n=prefs.getStringSet("progress",emptySet()).orEmpty().toMutableSet();n.add("$now|$weight|${waist?:""}|${safe(note)}|$now");prefs.edit().putStringSet("progress",n).apply()}\n    fun deleteProgress(id:Long){prefs.edit().putStringSet("progress",prefs.getStringSet("progress",emptySet()).orEmpty().filterNot{it.startsWith("$id|")}.toSet()).apply()}
    ...
    fun addInventory(peptide:String,vial:Double,quantity:Int,batch:String){val now=System.currentTimeMillis();val n=prefs.getStringSet("inventory",emptySet()).orEmpty().toMutableSet();n.add("$now|${safe(peptide)}|$vial|$quantity|${safe(batch)}");prefs.edit().putStringSet("inventory",n).apply()}
